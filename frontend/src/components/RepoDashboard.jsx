        ...
        <motion.div variants={itemVars} className="bg-zinc-900/80 border border-zinc-800 rounded-2xl p-6">
           ...
           <div className="space-y-4 overflow-y-auto max-h-[400px] custom-scrollbar pr-2">
             {data.recommendations?.map((rec, i) => (
                <div key={i} className="bg-zinc-800/40 border border-zinc-700/50 rounded-xl p-4 shadow-sm hover:border-zinc-600 transition">
                  <div className="flex justify-between items-start mb-3">
                     <h3 className="text-zinc-100 text-sm font-bold flex items-center gap-2"><span className="text-zinc-500 text-xs">{i+1}.</span> {rec.title}</h3>
                     <span className={`text-[9px] px-2 py-0.5 rounded-md uppercase tracking-wider font-bold ${rec.priority === 'High' ? 'bg-red-500/10 text-red-400 border border-red-500/20' : 'bg-yellow-500/10 text-yellow-400 border border-yellow-500/20'}`}>{rec.priority} Priority</span>
                  </div>
                  
                  <div className="space-y-2 mb-3">
                     <div className="bg-zinc-950/50 p-2.5 rounded-lg border border-zinc-800">
                        <span className="text-[10px] text-zinc-500 uppercase font-bold block mb-1">Recommended Fix</span>
                        <span className="text-xs text-zinc-300 leading-relaxed">{rec.fix}</span>
                     </div>
                     <div className="bg-green-500/5 p-2.5 rounded-lg border border-green-500/10">
                        <span className="text-[10px] text-green-500/70 uppercase font-bold block mb-1">Expected Business Benefit</span>
                        <span className="text-xs text-green-400/90 leading-relaxed">{rec.benefit}</span>
                     </div>
                  </div>

                  <div className="flex flex-wrap gap-2 text-[10px] uppercase font-bold">
                     <span className="text-zinc-400 border-r border-zinc-700 pr-2">Effort: <span className="text-zinc-200">{rec.effort}</span></span>
                     <span className="text-zinc-400 border-r border-zinc-700 pr-2">Impact: <span className="text-zinc-200">{rec.impact}</span></span>
                     <span className="text-indigo-400 border-r border-zinc-700 pr-2">ETA: <span className="text-indigo-300">{rec.eta}</span></span>
                     <span className="text-purple-400 flex items-center gap-1">{rec.owner}</span>
                  </div>
                </div>
             ))}
           </div>
