...
class RepoIntelligence:
    def __init__(self, repo_url, branch='main', mode='full'):
        self.repo_url = repo_url
        self.branch = branch
        self.mode = mode
        self.repo_name = repo_url.rstrip('/').split("/")[-1].replace(".git","")
        self.owner = repo_url.rstrip('/').split("/")[-2] if len(repo_url.split("/")) > 4 else "Unknown"
        self.local_path = f'./temp/{self.repo_name}'
        self.ignored = {'.git','node_modules','dist','build','venv','__pycache__','.next','.cache','coverage', '.pytest_cache', 'target', 'vendor'}
    ...
    def clone_repo(self):
        ...
        try:
            repo = git.Repo.clone_from(self.repo_url, self.local_path, branch=self.branch, depth=1)
            ...
            try:
                ...
                os.makedirs(self.local_path, exist_ok=True)
                repo = git.Repo.clone_from(self.repo_url, self.local_path, depth=1)
    ...
    def scan(self):
        ...
        for root, dirs, fs in os.walk(self.local_path):
            ...
            for f in fs:
                ...
                if ext in ['.js', '.ts', '.tsx', '.py', '.json', '.txt', ''] and files_cnt < 400:
                    try:
                        with open(os.path.join(root, f), 'r', encoding='utf-8') as file:
                            file_contents[rel_file] = file.read()
        ...
        recs = [
            {
                ...
                "fix": "All detected inline constants representing authentication keys must be extracted rapidly into a centralized secure store like AWS Secrets Manager or Kubernetes ConfigMaps.",
                "benefit": "Immediate neutralization of credential exploitation vectors and simpler rotation protocols."
            },
            {
                "priority": "High", "effort": "High", "impact": "Maintainability", "eta": str(max(1, folders_cnt // 2)) + " days", "owner": "Core Backend", 
                "title": "Refactor God Controller Operations", 
                "fix": "We detected excessive procedural logic bypassing service layers. Extract business validation from controllers and deploy discrete micro-commands.",
